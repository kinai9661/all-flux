...
document.getElementById('generateForm').addEventListener('submit',async function(e){
  ...
  btn.disabled=true;
  btn.textContent='🎨 生成中...';
  document.getElementById('results').innerHTML='<div class="loading"><div class="spinner"></div><p>正在生成圖像，請稍候...</p></div>';
  
  const sizePreset=document.getElementById('size').value;
  const sizeConfig=PRESET_SIZES[sizePreset];
  
  const refImagesText=document.getElementById('referenceImages').value.trim();
  const referenceImages=refImagesText?refImagesText.split(',').map(s=>s.trim()).filter(s=>s):[]; 
  ...
  try{
    ...
    if(response.ok){
      ...
      if(contentType&&contentType.startsWith('image/')){
        ...
        const seed=response.headers.get('X-Seed')||'0';
        const style=response.headers.get('X-Style')||'none';
        const width=response.headers.get('X-Width')||'1024';
        const height=response.headers.get('X-Height')||'1024';
        const qualityMode=response.headers.get('X-Quality-Mode')||'standard';
        ...
        if(data.data&&data.data.length>0){
          ...
          data.data.forEach(img=>addToHistory({
            url:img.image,
            prompt:prompt,
            model:img.model,
            seed:img.seed,
      ...
      document.getElementById('results').innerHTML='<div class="alert alert-error">生成失敗: '+error.error.message+'</div>';
    }
  }catch(e){
    document.getElementById('results').innerHTML='<div class="alert alert-error">請求失敗: '+e.message+'</div>';
  }finally{
    btn.disabled=false;
...
function displayImages(images,prompt){
  ...
  let html='<div class="prompt-display"><div class="label">提示詞</div><div class="content">'+prompt+'</div></div>';
  ...
  images.forEach(img=>{
    ...
    html+=\`<div class="gallery-item">
      <img src="\${img.image}" alt="Generated">
      <div class="gallery-info">
        ...
        <div class="gallery-meta" style="margin-top:5px">
          ...
          <span class="time-badge">\${img.width}x\${img.height}</span>
        </div>
        <div class="gallery-actions">
          <button class="action-btn" onclick="downloadImage('\${img.image}',\${img.seed})">💾 下載</button>
          <button class="action-btn" onclick="openModal('\${img.image}')">🔍 查看</button>
        </div>
      </div>
    </div>\`;
  });
  ...
  history.forEach(item=>{
    ...
    itemDiv.innerHTML=\`
      ...
      <div class="gallery-info">
        ...
        <div class="gallery-actions">
          <button class="action-btn" onclick="downloadImage('\${item.url}',\${item.seed})">💾</button>
          <button class="action-btn" onclick="openModal('\${item.url}')">🔍</button>
          <button class="action-btn delete" onclick="deleteFromHistory(\${item.id})">🗑️</button>
        </div>
      </div>
    \`;
...
function downloadImage(url,seed){
  const link=document.createElement('a');
  link.href=url;
  link.download='flux-ai-'+seed+'.png';
  link.click();
}
